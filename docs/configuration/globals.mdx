...
As of Payload version `0.15.0`, you can configure a Global to keep a running history of its changes over time (known as [versions](/docs/versions/overview)), including the ability to restore the global to a given version in its history. You can also use Versions to create a true [Draft](/docs/versions/drafts) mode for your Global as well as enable [Autosave](/docs/versions/autosave).

For more information about how to configure Versions for Globals, [click here](/docs/versions/overview#globals).
